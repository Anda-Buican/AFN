...
{
	int poz = 0, stare_curenta = 0;
	while (poz < cuv.length())
	{

		for (Tranzitie t : stari[stare_curenta].tranzitii)
		{
			if (cuv[poz] == t.c)
			{
				poz++;
				stare_curenta = t.dest;
				break;
			}
		}

	}
	if (stari_fin[stare_curenta])
		return 1;
	return 0;
